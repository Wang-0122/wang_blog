...
public class SystemMonitor {
	...
	public static double getCpuUsage() {
		...
		if (osName.toLowerCase().contains("windows") || osName.toLowerCase().contains("win")) {
			...
			try {
				Map<?, ?> map1 = SystemMonitor.cpuinfo();
				Thread.sleep(CPUTIME);
				Map<?, ?> map2 = SystemMonitor.cpuinfo();
	...
	public static double[] getMemUsage() {
		if (osName.toLowerCase().contains("windows") || osName.toLowerCase().contains("win")) {
			...
			try {
				...
				return new double[] { memoryUsage, freePhysicalMemorySize / (1024 * 1024) };
			} catch (Exception e) {
				logger.debug(e.getMessage());
			}
		}
			...
			try {
				...
				BigDecimal b1 = new BigDecimal(usage);
				double memoryUsage = b1.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
				return new double[] { memoryUsage, (memused - buffers - cached) / 1024 };
			} catch (Exception e) {
				logger.debug(e.getMessage());
			} finally {
				try {
	...
	public static double[] getDiskUsage() throws Exception {
		...
		if (osName.toLowerCase().contains("windows") || osName.toLowerCase().contains("win")) {
			...
			return new double[] { precent, allFree / (1024 * 1024 * 1024) };
		}
		else {
			...
			BigDecimal b1 = new BigDecimal(precent);
			precent = b1.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
			return new double[] { precent, usedHD / (1024 * 1024) };
		}
	...
	public static void main(String[] args) throws Exception {
		...
		// 当前系统的内存使用率
		double[] memUsage = SystemMonitor.getMemUsage();
