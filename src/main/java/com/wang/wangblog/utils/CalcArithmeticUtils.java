 ...
 * 
 */
public class CalcArithmeticUtils {
    ...
    public static double excute(String value, Object object) throws Exception {
        return excute(value, object, null);
    }

    @SuppressWarnings("unchecked")
    public static double excute(String value, Object object, String[] specifiedProperties) throws Exception {
        List<String> specifiedList = (specifiedProperties != null) ? Arrays.asList(specifiedProperties) : null;

        if (object instanceof java.util.Map) {
            java.util.Map<String, ?> objMap = (java.util.Map<String, ?>) object;
            for (String key : objMap.keySet()) {
                if (specifiedList.contains(key)) {
                    value = value.replaceAll(key, objMap.get(key).toString());
                }
            }
            return excute(value);
        } else if (object != null) {
            PropertyDescriptor[] targetPds = BeanUtils.getPropertyDescriptors(object.getClass());
            for (PropertyDescriptor targetPd : targetPds) {
                if (targetPd.getWriteMethod() != null && specifiedList.contains(targetPd.getName())) {
                    Method readMethod = targetPd.getReadMethod();
                    if (!Modifier.isPublic(readMethod.getDeclaringClass().getModifiers())) {
                        readMethod.setAccessible(true);
                    }
                    Object eleValue = readMethod.invoke(object);
                    if (eleValue != null) {
                        if (eleValue instanceof Integer || eleValue instanceof Long || eleValue instanceof Double || eleValue instanceof Float || eleValue instanceof BigDecimal) {
                            value = value.replaceAll(targetPd.getName(), eleValue.toString());
                        }
                    }
                }
            }
